import {
    ...
    GET_USER_BY_EMAIL,
    GET_ALL_CATEGORIES,
    GET_TOPICS_BY_CATEGORY_ID,
    INSERT_INTO_USER_PROFILE,
    GET_USER_PROFILE_BY_USER_ID,
    INSERT_INTO_COMMUNITIES,
    GET_COMMUNITY_BY_ID,
    GET_LATEST_COMMUNITY_BY_OWNER_ID,
    GET_ALL_COMMUNITIES,
    GET_TOPICS_BY_USER_ID,
    GET_COMMUNITIES_BY_USER_ID,
    GET_USERS_BY_COMMUNITY_ID,
    ADD_USER_TO_TOPIC,
    ...
    GET_POST_BY_ID,
    GET_ALL_TOPICS,
    GET_POSTS_BY_USER_AND_COMMUNITY,
...
const resolvers = {
    Query: {
        ...
        verifyOtp: async (_: any, { user_id, otp }: { user_id: number, otp: string }, context: any) => {
            const otpRecord = await context.db.get(
                GET_OTP_BY_USER_ID,
                [user_id]
            );
            if (!otpRecord) {
        ...
        topics: async (_: any, __: any, context: any) => {
            return context.db.all(GET_ALL_TOPICS);
        },

        communities: async (_: any, __: any, context: any) => {
            return context.db.all(GET_ALL_COMMUNITIES);
        },

        community: async (_: any, { community_id }: { community_id: string }, context: any) => {
            return context.db.get(GET_COMMUNITY_BY_ID, [community_id]);
        },

        posts: async (_: any, { community_id }: { community_id: string }, context: any) => {
            if (!context.auth.user) {
                throw new Error('Not authenticated');
            }
    ...
    Mutation: {
        ...
        createProfile: async (_: any, { createProfileInput }: { createProfileInput: CreateProfileInput }, context: any) => {
            if (!context.auth.user) {
                throw new Error('Not authenticated');
            }
            try {
        ...
        generateOtp: async (_: any, { mobile_number }: { mobile_number: string }, context: any) => {
        ...
        createCommunity: async (_: any, { createCommunityInput }: { createCommunityInput: CreateCommunityInput }, context: any) => {
            if (!context.auth.user) {
                throw new Error('Not authenticated');
            }
            await context.db.run(
        ...
        createPost: async (_: any, { createPostInput }: { createPostInput: CreatePostInput }, context: any) => {
            if (!context.auth.user) {
                throw new Error('Not authenticated');
            }
            await context.db.run(
